...
ConvexMPCLocomotion::ConvexMPCLocomotion(float _dt, int _iterations_between_mpc, MIT_UserParameters *parameters) :
        ...
        horizonLength(10),
        dt(_dt),
        trotting(int( 10 * 2.0 ), Vec4<int>(0, 10, 10, 0), Vec4<int>(12, 12, 12, 12), "Trotting"),
        ...
        random2(horizonLength, Vec4<int>(8, 16, 16, 8), 0.5, "Double Trot")
{
    ...
    setup_problem(dtMPC, horizonLength, 0.4, 300);//120
...
void ConvexMPCLocomotion::run(ControlFSMData<float> &data)
{
    ...
    if(firstRun)
    {
        ...
        world_position_desired[2] = seResult.position[2];//seResult.rpy[2];
        _yaw_des = seResult.rpy[2];

        for(int i = 0; i < 4; i++)
        {
            
            footSwingTrajectories[i].setHeight(0.10);//0.05
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        footSwingTrajectories[i].setHeight(.10);//0.05
...
//            0, 700, 0,
//            0, 0, 150;
    Kp << 600, 0, 0,         /* 700 , 0 , 0 ; 0 , 700 , 0 ; 0 , 0 , 400*/ // Control 
            0, 600, 0,          
            0, 0, 600;              
...
void ConvexMPCLocomotion::solveDenseMPC(int *mpcTable, ControlFSMData<float> &data)
{
   ...
   //float Q[12] = {0.25, 0.25, 10, 2, 2, 50, 0, 0, 0.3, 0.2, 0.2, 0.1};
    float Q[12] = {1.25, 1.25, 2, 2, 2, 50, 0, 0, 0.3, 1.5, 1.5, 0.2};//2
    ...
    Timer t1;
    dtMPC = dt * iterationsBetweenMPC;
    setup_problem(dtMPC, horizonLength, 0.4, 120);//120
    //setup_problem(dtMPC,horizonLength,0.4,650); //DH
...
void ConvexMPCLocomotion::initSparseMPC()
{
    ...
    _sparseCMPC.setFriction(1.0);//0.4
